...
class dVVKSigningFlow2Step {
    ...
    async setRequest(request) {
        if (!(request instanceof BaseTideRequest_1.default))
    ...
    async preSign(dynamicData) {
        ...
        const voucherFlow = new VoucherFlow_1.default(this.orks.map(o => o.orkPaymentPublic), this.voucherURL, this.vendorAction);
    ...
    async sign(dynamicData) {
        ...
        const pre_SignResponses = this.preSignState.clients.map((client, i) => client.Sign(this.vvkid, dynDataisArray ? this.request.replicate().setNewDynamicData(dynamicData[i]) : this.request, this.preSignState.GRj, (0, Serialization_1.serializeBitArray)(this.preSignState.bitwise)));
        ...
        for (let i = 0; i < this.preSignState.GRj.length; i++) {
            sigs.push((0, Serialization_1.ConcatUint8Arrays)([this.preSignState.GRj[i].toRawBytes(), (0, Serialization_1.BigIntToByteArray)(Sj[i])]));
        }
        return {
            sigs,
