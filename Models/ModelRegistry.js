...
class PolicySignRequestBuilder extends HumanReadableModelBuilder {
    ...
    _isUint8Array(obj) {
        ...
        if (ArrayBuffer.isView(obj) && obj.constructor?.name === 'Uint8Array') return true;
        if (typeof obj === 'object' && typeof obj.length === 'number' &&
            obj.buffer instanceof ArrayBuffer && typeof obj.byteOffset === 'number') return true;
        return false;
    }

    // Helper to ensure we have a plain LOCAL Uint8Array
    // Cross-realm ArrayBuffers also cause issues, so we copy byte by byte
    _toUint8Array(data) {
        if (!data) return null;
        if (data instanceof Uint8Array) return data;
        // For cross-realm arrays, copy byte by byte for maximum compatibility
        ...
        console.error('[_toUint8Array] Invalid data:', typeof data, data);
        return null;
    }
    ...
    getDetailsMap() {
        ...
        // Convert draft to plain Uint8Array
        const draftBytes = this._toUint8Array(this._draft);
        if (!draftBytes) return { error: 'No draft data' };

        const policyBytes = this._toUint8Array(GetValue(draftBytes, 0));
        const policy = new Policy(policyBytes);
        ...
        policy.params.entries().forEach(([key, value]) => {
            if (!this._isUint8Array(value)) summary[`Parameter:${key}`] = value;
        });

        let res = {};
        if (TryGetValue(draftBytes, 1, res)) {
            const contractBytes = this._toUint8Array(res.result);
            const contractType = StringFromUint8Array(this._toUint8Array(GetValue(contractBytes, 0)));
    ...
    getRequestDataJson() {
        ...
        if (TryGetValue(draftBytes, 1, res)) {
            ...
            if (TryGetValue(contractBytes, 1, forsetiDataRes)) {
                const forsetiData = this._toUint8Array(forsetiDataRes.result);

                // forsetiData[1] = innerPayload
                let innerPayloadRes = {};
                if (TryGetValue(forsetiData, 1, innerPayloadRes)) {
                    const innerPayload = this._toUint8Array(innerPayloadRes.result);

                    // innerPayload[0] = sourceCode
                    let sourceCodeRes = {};
                    if (TryGetValue(innerPayload, 0, sourceCodeRes)) {
                        const contractCode = StringFromUint8Array(this._toUint8Array(sourceCodeRes.result));
